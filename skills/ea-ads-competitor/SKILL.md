---
name: ea-ads-competitor
description: >
  Competitor ad intelligence analysis across Google and Meta. Analyzes
  competitor ad copy, creative strategy, keyword targeting, estimated spend,
  and identifies competitive gaps and opportunities. MCPs utiles : meta-ads
  (search_ad_library pour la veille Meta/IG), google-ads-gms (auction insights).
...
- **Cadre global** : `ea-ads` contient les quality gates et benchmarks transversaux
- **Benchmarks** : `ea-ads/references/benchmarks.md` pour les baseline CPC/CTR/CVR par industrie
- **MCP Meta** : `meta-ads` — `search_ad_library` pour les créas concurrentes sur Meta/Instagram
- **MCP Google** : `google-ads-gms` — Auction Insights via GAQL

## Process

1. Identifier les concurrents cibles (input utilisateur ou analyse industrie)
2. Charger `ea-ads/references/benchmarks.md` pour les baselines CPC/CTR/CVR
3. Rechercher les ads concurrentes via MCP `meta-ads` → `search_ad_library`
4. Analyser le copy, les créas, et les thèmes messaging
5. Estimer les dépenses et la stratégie keywords
6. Identifier les gaps et opportunités
7. Générer le rapport d'intelligence concurrentielle

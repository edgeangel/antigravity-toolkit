...
description: >
  ...
  MCPs utiles : meta-ads (insights, execute, ad library, targeting).
...
- **MCP** : `meta-ads` (get_insights, execute, search_ad_library, search_targeting)

## Process

1. Collecter les données via MCP `meta-ads` (get_insights) ou export Ads Manager
2. Charger `ea-ads/references/meta-audit.md` pour la checklist 46 checks
3. Charger `ea-ads/references/benchmarks.md` pour les benchmarks Meta
4. Charger `ea-ads/references/scoring-system.md` pour le scoring pondéré
5. Évaluer chaque check : PASS, WARNING, ou FAIL
6. Calculer le Meta Ads Health Score (0-100)
7. Générer le rapport avec plan d'action

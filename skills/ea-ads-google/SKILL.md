...
description: >
  ...
  wasted spend, account structure, keywords, ads, and settings. MCPs utiles :
  google-ads-gms (GAQL queries, reporting), google-analytics (attribution,
  conversions). Use when user says "Google Ads", "Google PPC", "search ads",
  "PMax", "Performance Max", or "Google campaign".
---
...
- **Cadre global** : `ea-ads` contient les quality gates, industry detection et scoring transversaux
- **Créa cross-plateforme** : `ea-ads-creative` pour la fatigue detection et diversité formats
- **Landing pages** : `ea-ads-landing` pour l'audit post-clic
- **Contexte client** : `ea-client-context` si pertinent
- **MCPs** : `google-ads-gms` (GAQL queries), `google-analytics` (attribution)

## Process

1. Collecter les données via MCP `google-ads-gms` ou export CSV
2. Charger `ea-ads/references/google-audit.md` pour la checklist 74 checks
3. Charger `ea-ads/references/benchmarks.md` pour les benchmarks Google
4. Charger `ea-ads/references/scoring-system.md` pour le scoring pondéré
5. Évaluer chaque check : PASS, WARNING, ou FAIL
6. Calculer le Google Ads Health Score (0-100)
...
- Mobile page speed: test via browser → PageSpeed Insights (`pagespeed.web.dev`)
- Schema markup: test via browser → Rich Results Test (`search.google.com/test/rich-results`)
- Landing page relevance to ad copy and keywords
- For e-commerce: verify Product schema has `offers`, `review`, `aggregateRating`
